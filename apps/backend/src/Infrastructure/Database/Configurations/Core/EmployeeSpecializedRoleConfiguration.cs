...
public class EmployeeSpecializedRoleConfiguration : IEntityTypeConfiguration<EmployeeSpecializedRole>
{
    public void Configure(EntityTypeBuilder<EmployeeSpecializedRole> builder)
    {
        ...
        builder.HasOne(es => es.EmployeeProfile)
            .WithMany(e => e.SpecializedRoles)
            .HasForeignKey(es => es.EmployeeProfileId);

        builder.HasOne(es => es.SpecializedRole)
            .WithMany()
